...
ttt=tiledlayout(1,1,'TileSpacing','compact','Padding','compact');
global Pres Temp rho  cv uGas co2 h2o QH2O QCO2 Qc rho_c Composition beta rho_t np nt CH2O  Xf rhoGas rhoCrystal alphaFig betaFig
...
cv=griddedInterpolant(P,X,T-273.15,Cv);
uGas = griddedInterpolant(P,X,T-273.15,U);
...
Temperture(1:nt)=linspace(500,950,nt);
[Pres,Temp]=ndgrid(Presure,Temperture);
[rho_t,U,alpha,beta,CH2O,XH2O]=den(Pres,Temp);
...
plot(Pres,U,'linewidth',2);
set(gca,'FontSize',12, 'Fontweight', 'bold','linewidth',1.5)
ylabel('int. energy density, J/m^3')
xlabel('Pressure, MPa')
legendStr = string(squeeze(T(1,1,:)));
legend(legendStr)
hold on
...
function [rho_t,U,alpha,beta,CH2O,XH2O]=den(P,T)
global rho cv uGas co2 h2o Xf rho_c QH2O QCO2 Qc rhoGas rhoCrystal alphaFig betaFig
CCO2=co2(P);
CH2O=h2o(P);
XH2O=Xf(P);
XH2Od=CH2O./(CH2O+CCO2);
beta=betaeq(P,XH2Od,T);
...
U=((rhoCrystal .* Cv_s + rhoMelt .* Cv_m) .* (T+273.15) + rhoGas .* U_g)./ rho_t;
end
...
beq=min(1,max(0,1-(2.92*Theta-4.76*Theta.^2+2.84*Theta.^3)));
end