...
global Pres Temp rho co2 h2o QH2O QCO2 Qc rho_c Composition beta rho_t np nt CH2O rho_crystal
...
%%parameters
DenVisFilename = 'denvis_40000.mat';
np=50;                                % #points in pressure array
nt=5;
Temp0=850;                               %temperature in C             range [800  900]
Psat=939.84;                              %Saturatiom pressure in MPa
CH2O_sat=0.0624;                          %saturation magma water content range [0.03-0.1]
...
Presure=linspace(25,Psat,np);
Temperture(1:nt)=linspace(775,900,nt);
[Pres,Temp]=ndgrid(Presure,Temperture);
X0(1,1:np,1:nt)=0.01;
X0(2,1:np,1:nt)=xH2O;

xx=fsolve(@initcond,X0,options);
alpha(1:np,1:nt)=xx(1,:,:);
xw(1:np,1:nt)=xx(2,:,:);
...
hold on
plot(pp, aa, 'linewidth',1, 'LineStyle','--')
...
hold on
plot(pp, xh2o, 'linewidth',1, 'LineStyle','--')
set(gca,'FontSize',12, 'Fontweight', 'bold','linewidth',1.5)
ylabel('XH2O')
xlabel('Pressure, MPa')
...
hold on
plot(pp, bb, 'linewidth',1, 'LineStyle','--')
...
hold on
plot(pp, rr, 'linewidth',1, 'LineStyle','--')
set(gca,'FontSize',12, 'Fontweight', 'bold','linewidth',1.5)
...
for iP=1:np
    for iT=1:nt
        muu= Giordano_2008_visc(CH2O(iP,iT)'*100, Temperture(iT)+273.15, Composition)'.*theta(beta(iP,iT));
        mu(iP,iT)=muu;
    end
end
semilogy(Pres,mu,'linewidth',2);
hold on
semilogy(pp, mm, 'linewidth',1, 'LineStyle','--')
...
Pres = 1e6 * Pres;
% save(DenVisFilename, "Pres", "Temp", "mu", "rho_t", "rho_crystal");
...
function F=initcond(X)
    global Pres Temp rho co2 h2o QH2O QCO2 Qc rho_c beta rho_t np nt CH2O rho_crystal
    alpha(1:np,1:nt)=X(1,:,:);
    XH2O(1:np,1:nt)=X(2,:,:);
    CCO2=co2(Pres,XH2O);
    CH2O=h2o(Pres,XH2O);
    ...
    rho_crystal = rho_c.*(1-alpha).*beta;
    rhoh2o=rho_m.*(1-alpha).*(1-beta).*CH2O+rho_g.*XH2O.*alpha;
    rhoco2=rho_m.*(1-alpha).*(1-beta).*CCO2+rho_g.*(1-XH2O).*alpha;
    F(1:np,1:nt,1)=rhoh2o./rhoc-QH2O/Qc*ones(np,nt);
    F(1:np,1:nt,2)=rhoco2./rhoc-QCO2/Qc*ones(np,nt);
...
function beq=betaeq(Pm,XH2Od,T)
    ...
    beq=min(1,max(0.0,1-(2.92*Theta-4.76*Theta.^2+2.84*Theta.^3)));
end