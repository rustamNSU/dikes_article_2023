...
%% Different k1c
% simDirs = ["simID_100000", "simID_100001", "simID_100002"];
% legendText = [
%     "$K_{1c}\!=\!1$ MPa$\cdot$m$^{1/2}$", ... 
%     "$K_{1c}\!=\!100$ MPa$\cdot$m$^{1/2}$", ...
%     "$K_{1c}\!=\!500$ MPa$\cdot$m$^{1/2}$"];

% %% Convergence Nx
% simDirs = ["simID_110021", "simID_110022", "simID_110000"];
% legendText = [
%     "$N_x=250$", ...
%     "$N_x=500$", ... 
%     "$N_x=1000$"];
% saveFilename = 'compare_convergence_Nx_host_rock_on.pdf';
% lineMarkers = {'none', 'none', 'none', 'none'};
% simColors = {'b', 'k', 'r', 'g'};
% timestep = 600;
% xLim = [-35, -5];

% %% Convergence Nx host rock off
% simDirs = ["simID_120002", "simID_120000", "simID_120001"];
% legendText = [
%     "Without host rock $N_x=250$", ...
%     "Without host rock $N_x=500$", ...
%     "Without host rock $N_x=1000$"];
% saveFilename = 'compare_convergence_Nx_host_rock_off.pdf';
% lineMarkers = {'none', 'none', 'none', 'none'};
% simColors = {'b', 'k', 'r', 'g'};
% timestep = 625;
% xLim = [-35, -5];
...
%% Bubbule relative viscosity
% simDirs = ["simID_142000", "simID_142001", "simID_142002"];
% legendText = [ 
%     "$\left.\eta_r=0.5\right|_{\alpha=0.3}$", ...
%     "$\eta_r=1$", ...
%     "$\left.\eta_r=1.5\right|_{\alpha=0.3}$"];
% saveFilename = 'subplots_viscosity.pdf';
% timestep = 400;
% xLim = [-35, 0];
% lineMarkers = {'none', 'none', 'none', 'none'};
% simColors = {'b', 'k', 'r', 'g'};
...
% %% Different XH20
% simDirs = ["simID_10121", "simID_1000"];
% legendText = ["mixed H$_2$O--CO$_2$", "H$_2$O only"];
% saveFilename = 'compare_xh2o_parameters.pdf';
% timestep = 70;
% xLim = [-35, 0];
% simColors = {'k', 'r'};
...
% saveFilename = 'subplots_toughness.pdf';
% stLegendFontSize = 8;
% timestep = 350;
% xLim = [-35, 0];
% lineMarkers = {'none', 'none', 'none', 'none'};
...
%     "$C_{SiO_2} = 64.6$ wt\%", ...
%     "$C_{SiO_2} = 70.6$ wt\%"];
% saveFilename = 'compare_sio2_TL.pdf';
% timestep = 720;
% lineMarkers = {'none', 'none', 'none', 'none'};
% simColors = {'k', 'b', 'g', 'r'};
% xLim = [-35, 0];
...
%% h2o-co2 mixed vs h2o only
simDirs = ["simID_143001", "simID_143009"];
legendText = [
    "mixed H$_2$O-CO$_2$", ...
    "H$_2$O only"];
saveFilename = 'compare_mixed_h2o-co2.pdf';
timestep = 720;
lineMarkers = {'none', 'none', 'none', 'none'};
simColors = {'k','r'};
xLim = [-35, 0];
...
legend(ax1, hLine1, legendText, Location='best', FontSize=stLegendFontSize)
legend(ax2, hLine2, "bubble fraction, $\alpha$", "crystal content, $\beta$", Location='best', FontSize=stLegendFontSize)
