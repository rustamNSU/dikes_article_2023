function debug_fracture_data(fractureData, reservoir)
    %% Fracture parameters
%     frac_elem = fractureData.get_fracture_elements();
    ...
    beta      = fractureData.beta(frac_elem);
    ...
    line_elastic_pressure = plot(subplotElasticPressure, X, beta, 'linewidth', 2);
    xlabel('Depth [km]');
    ylabel('beta');
    ylim([0, 1]);
    grid on;
